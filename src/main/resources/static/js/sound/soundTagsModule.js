// 검색에 대한 태그 랜더링과 비즈니스 로직 구현
        ...
        function renderTagsFromSearch(data, initialParams = {}) {
            ...
            const html = `
                <div class="music-tag-sort">
                    ...
                    <div class="music-tag-display">
                        <div class="tag-section hidden" id="instrument-section">${data.dto.instrument.map(tag => createTagItem(tag, 'instrument')).join('')}</div>
                        <div class="tag-section hidden" id="mood-section">${data.dto.mood.map(tag => createTagItem(tag, 'mood')).join('')}</div>
                        <div class="tag-section hidden" id="genre-section">${data.dto.genre.map(tag => createTagItem(tag, 'genre')).join('')}</div>
                    </div>
                </div>
            `;
            ...
            function performSearch(params, isDeactivation = false) {
                // 현재 URL의 쿼리 파라미터를 파싱
                const currentParams = new URLSearchParams(window.location.search);
