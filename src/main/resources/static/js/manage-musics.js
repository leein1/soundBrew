    ...
    async function applyChanges(button, id) {
        ...
        const musicTitleInput = row.querySelector('.editable-field[data-field="musicTitle"]');
        const musicDescriptionInput = row.querySelector('.editable-field[data-field="musicDescription"]');

        // 선택된 라디오 버튼의 값 가져오기
        const soundTypeRadioChecked = row.querySelector('.editable-field[data-field="soundType"] input[type="radio"]:checked');
        const soundTypeValue = soundTypeRadioChecked ? soundTypeRadioChecked.value : '';

        // 현재 값을 표시할 요소들 가져오기
        const musicTitleDisplay = row.querySelector('.current-value[data-field="musicTitle"]');
        const musicDescriptionDisplay = row.querySelector('.current-value[data-field="musicDescription"]');
        const soundTypeDisplay = row.querySelector('.current-value[data-field="soundType"]');

        // 입력 필드에서 수정된 값을 화면에 표시
        musicTitleDisplay.textContent = musicTitleInput.value;
        musicDescriptionDisplay.textContent = musicDescriptionInput.value;
        soundTypeDisplay.textContent = soundTypeValue;

        // 입력 필드는 숨기고, 변경된 값이 표시된 부분은 다시 보여줌
        musicTitleInput.style.display = 'none';
        musicTitleDisplay.style.display = 'inline';
        musicDescriptionInput.style.display = 'none';
        musicDescriptionDisplay.style.display = 'inline';
        row.querySelector('.editable-field[data-field="soundType"]').style.display = 'none';
        soundTypeDisplay.style.display = 'inline';
        ...
        try {
            ...
            await fetch(`/sounds/manage/${id}`, {
                ...
                body: JSON.stringify({
                    ...
                    musicDescription: musicDescriptionInput.value,
