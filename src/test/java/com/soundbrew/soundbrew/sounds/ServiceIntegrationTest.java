...
public class ServiceIntegrationTest {
    ...
    public void testUpdateLinkTagsForAdmin_Success() {
        ...
        tagsDTO.setMood(Arrays.asList("sad"));         // 예: "sad" 태그만 연결
        ...
        assertEquals("sad", moodTags.get(0).getMoodTag().getMoodTagName(), "Mood 태그 이름이 'sad'여야 합니다.");
    ...
    public void testUpdateInstrumentTagSpelling() {
        ...
        ResponseDTO response = tagsService.updateInstrumentTagSpelling(beforeName, afterName);
    ...
    public void testUpdateMoodTagSpelling() {
        String beforeName = "sad";
        String afterName = "changetestsad";

        ResponseDTO response = tagsService.updateMoodTagSpelling(beforeName, afterName);
    ...
    public void testUpdateGenreTagSpelling() {
        ...
        ResponseDTO response = tagsService.updateGenreTagSpelling(beforeName, afterName);
    ...
    void updateMusicForArtist_Bad(){
        ...
        assertThrows(AccessDeniedException.class, () -> {
            soundsService.updateMusicForArtist(musicId,musicDTO, 1);
        }, "잘못된 사용자가 음원을 수정하려 하면 예외가 발생해야 합니다.");
    }
    @Test
