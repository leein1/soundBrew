...
//
//        // albumRepository 모킹: save 호출 시 전달된 엔티티 그대로 반환
//        when(albumRepository.save(any())).thenAnswer(invocation -> {
//            Object albumEntity = invocation.getArgument(0);
//            return albumEntity;
//        });
//
//        // musicRepository 모킹: save 호출 시 전달된 엔티티 그대로 반환
//        when(musicRepository.save(any())).thenAnswer(invocation -> {
//            Object musicEntity = invocation.getArgument(0);
//            return musicEntity;
//        });
//
//        // albumMusicRepository 모킹: save 호출 시 전달된 객체 그대로 반환
//        when(albumMusicRepository.save(any())).thenAnswer(invocation -> invocation.getArgument(0));
//
//        // tagsService 모킹: linkTags는 ResponseDTO를 반환하므로, 적절한 더미 값을 반환하도록 처리
//        when(tagsService.linkTags(any(Music.class), any(TagsDTO.class)))
//                .thenReturn(ResponseDTO.withMessage().message("음원의 태그를 새롭게 연결했습니다.").build());
//
//        // Act: 서비스 메소드 호출
...
//        TagsDTO tagsDTO = new TagsDTO();
//
//        User dummyUser = User.builder().userId(userId).nickname("dummyNickname").build();
//        when(userRepository.findById(anyInt())).thenReturn(Optional.of(dummyUser));
//
//        when(albumRepository.save(any())).thenThrow(new DataIntegrityViolationException("Missing mandatory fields"));
//
